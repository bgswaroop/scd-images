...
class SigNetFlow(object):
    ...
    @staticmethod
    def train_batch(inputs, targets, scaler):
        with autocast():
            SigNet.optimizer.zero_grad()
            outputs = SigNet.model(inputs)
            embeddings = SigNet.model.extract_features(inputs)
            loss = SigNet.criterion(outputs, targets)

        scaler.scale(loss).backward()
        scaler.step(SigNet.optimizer)
        scaler.update()
    ...
    def train(cls):
        ...
        train_loader = Data.load_data(config_file=Configure.train_data_config, config_mode='train',
                                      dataset_dir=Configure.dataset_folder)
        test_loader = Data.load_data(config_file=Configure.test_data_config, config_mode='test',
                                     dataset_dir=Configure.dataset_folder)

        # Generate the target labels (avg_fft)
        fft = {label_id: [] for label_id in range(18)}  # 18 here corresponds to the number of camera models
        avg_fft = {label_id: [] for label_id in range(18)}  # 18 here corresponds to the number of camera models
        for input_batch, (label_ids, img_paths) in train_loader:
            for img, label_id in zip(input_batch, label_ids):
                fft[int(label_id)].append(img)
        for label_id in fft:
            avg_fft[int(label_id)] = torch.mean(torch.stack(fft[int(label_id)]), dim=0)
        # visualize_ffts(fft, avg_fft)
        ...
        for epoch in range(init_epoch, SigNet.max_epochs + 1):
            ...
            # Early stopping
            stop_early, stop_early_message = TrainingUtils.early_stopping(
                patience=5, metric=history['val_loss'], delta=0.0001)
            if stop_early:
            ...
            for i, (images, (label_ids, img_paths)) in enumerate(train_loader):

                ae_targets = torch.stack([avg_fft[int(label_id)] for label_id in label_ids])
                stats, embeddings = cls.train_batch(images.unsqueeze(1).to(Configure.device),
                                                    ae_targets.unsqueeze(1).to(Configure.device), scaler)
            ...
            for i, (images, (label_ids, img_paths)) in enumerate(test_loader):
                ae_targets = torch.stack([avg_fft[int(label_id)] for label_id in label_ids])
                stats = cls.test_batch(images.unsqueeze(1).to(Configure.device),
                                       ae_targets.unsqueeze(1).to(Configure.device))
                loss[i], acc[i] = stats.loss, stats.acc
                # if i % 2 == 0:
                #     break
    ...
    def extract_signatures(cls, config_mode, images_dir=None, pre_trained_model_path=None):
        ...
        SigNet.model.eval()
        signatures = []
        for images, (label_ids, img_paths) in data_loader:
            images = images.unsqueeze(1).to(Configure.device)
            features = SigNet.model.extract_features(images).to(torch.device("cpu")).detach().numpy()
            labels = [Path(x).parent.name for x in img_paths]
            signatures += list(zip(features, labels))
    ...
    def classify(cls, config_mode='test', pre_trained_model_path=None):
        ...
        for idx, (images, (labels, img_paths, img_std_dev)) in enumerate(data_loader):
            ...
            loss[idx], acc[idx], ground_truths[idx], predictions[idx], pred_scores[idx] = \
    ...
    def predict(cls, prediction_data, pre_trained_model_path=None):
        ...
        SigNet.model.load_state_dict(params['model_state_dict'])
        SigNet.model.eval()
        data_loader = Data.load_data(config_file=prediction_data, config_mode='test',
                                     dataset_dir=Configure.dataset_folder)
