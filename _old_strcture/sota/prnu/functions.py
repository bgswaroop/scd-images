...
def extract_multiple(images: list, levels: int = 4, sigma: float = 5, processes: int = None,
                     batch_size=cpu_count(), tqdm_str: str = '') -> np.ndarray:
    """
    Extract PRNU noise from the list of images. Images must be of the same size and properly oriented
    :param tqdm_str: tqdm description (see tqdm documentation)
    :param batch_size: number of parallel processed images
    :param processes: number of parallel processes
    :param images: list of images of size (height , width, channels) and type np.uint8
    :param levels: number of wavelet decomposition levels
    :param sigma: estimated noise power
    :return: PRNU
    """
    ...
    if processes is None or processes > 1:
        ...
        for batch_idx0 in tqdm(np.arange(start=0, step=batch_size, stop=len(images)), disable=tqdm_str == '',
        ...
        for im in tqdm(images, disable=tqdm_str is None, desc=tqdm_str, dynamic_ncols=True):
...
def center_crop(array: np.ndarray, sizes: tuple) -> np.ndarray:
    """
    Crop a multi-dimensional array about its center, according to sizes
...
def gt(l1: list or np.ndarray, l2: list or np.ndarray) -> np.ndarray:
    ...
    for l1_idx, l1_sample in enumerate(l1):
        gt_arr[l1_idx, l2 == l1_sample] = True
