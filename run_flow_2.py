...
if __name__ == '__main__':
    ...
    elif patches_type == 'tr_hist_eq' or patches_type == 'tr_gamma_enc_2.2' or patches_type == 'tr_gamma_dec_2.2':
        Configure.test_data_config = Path(
            rf'/data/p288722/dresden/test/18_models_{patches_type}_128x128_{num_patches}/fold_{fold_id}.json')
        Configure.train_data_config = Configure.test_data_config
        Configure.dataset_folder = \
            rf'/scratch/p288722/datasets/dresden/source_devices/nat_patches_{patches_type}_128x128_20'
        Configure.runtime_dir = Path(
            rf'/scratch/p288722/runtime_data/scd_pytorch/18_models_hcal_{patches_type}_1/test_{num_patches}/fold_{fold_id}')
        Configure.update()
        source_root = Path(rf'/scratch/p288722/runtime_data/scd_pytorch/18_models_hcal_09/test_1/fold_{fold_id}/')
    ...
    elif patches_type == 'train_homo_test_random':
        Configure.test_data_config = Path(
            rf'/data/p288722/dresden/test/18_models_random_128x128_{num_patches}/fold_{fold_id}.json')
        Configure.train_data_config = Configure.test_data_config
        Configure.dataset_folder = \
            rf'/scratch/p288722/datasets/dresden/source_devices/nat_patches_random_128x128_400'
        Configure.runtime_dir = Path(
            rf'/scratch/p288722/runtime_data/scd_pytorch/18_models_hcal_{patches_type}_3/test_{num_patches}/fold_{fold_id}')
        Configure.update()
        source_root = Path(rf'/scratch/p288722/runtime_data/scd_pytorch/18_models_hcal_09/test_1/fold_{fold_id}/')

    else:
        raise ValueError('Invalid patches_type')
